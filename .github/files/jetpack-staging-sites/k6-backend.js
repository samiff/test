...
export default function () {
	...
	sites.forEach( site => {
		group( `Backend tests for site: ${ site.url } ( ${ site.blog_id } )`, () => {
            ...
            res = http.get( `${ site.url }/wp-json/wp-site-health/v1/tests/dotorg-communication`, params );
			check( res, {
				'status was 200': r => r.status == 200,
                'verify connection message': r => r.body.includes( 'Can communicate with WordPress.org' ),
			} );
		} );
	} );
}
